...
4. ~~`#course` 코스 선택 페이지~~ → **빙고 페이지 상단 탭으로 통합됨** (퇴사/혼자/데이트 탭, `selectCourse()`). 저장안내에서 바로 빙고 진입(기본 코스 quit). 진행 중 탭 전환 시 confirm으로 초기화 보호. #course 섹션은 DOM에 남아있으나 미사용. 자율선택(free)은 코드만 유지
