...
4. **단일 빙고판 구조 (2026-08-04 확정)**: 코스별 3판이 아니라 **한 판**. `BOARD_FIXED=[embro,holly,star | chili,junco,yoon | eunh,kiko,wink]` — 1줄(0-2) 혼자 / 2줄(3-5) 퇴사 / 3줄(6-8) 데이트. 상단 탭(혼자/퇴사/데이트)은 판을 바꾸지 않고 **해당 줄을 하이라이트**(.row-hi 오렌지 테두리)만 함. 저장안내 → 바로 빙고 진입(`initBoard()`). state에 `layout:'mixed1'` 버전 필드 — 구버전 상태는 무시하고 새 판으로 시작. #course 섹션·COURSES·자율선택은 코드만 잔존(미사용). 미배치 업체 4곳(remem/barun/theone/mira)은 빙고판에 없고 업체 리스트에만 노출
5. `#bingo` 빙고 — 3×3판(프레임형, 이모지 아이콘+흑백처리), 참여업체 13곳 리스트, 쿠폰 바텀시트(바코드), 진행상황 3박스, 응모권 배지

### 빙고/응모 규칙 (0805 개정 — 최신)
- **빙고판 = 자율+고정 칸**: `CELL_DEFS` — 0번 칸 '음식점', 5번 칸 '카페 및 서비스', 7번 칸 '술집'(카테고리 고정, 해당 업종만 선택 가능·`STORE_CATS` 매핑), 나머지 6칸 자유 선택. 시작 시 **나만의 코스명 입력 모달**(예: 퇴근 후 기분전환 코스) → 상단 코스명 바에 노출(탭하여 수정). 배치 후 스탬프 전이면 쿠폰 시트의 "다른 업체로 변경"으로 교체 가능. state layout='custom1'(courseName 포함, 구버전 무시)
- **응모권 규칙(변경)**: n번째로 완성한 줄 = n장 (1줄→1, 2줄째→2, 3줄째→3…, 8줄 전체=36장, 상한 없음). 동시 다줄 완성 시 각 줄의 순번 합만큼 한 장씩 순차 응모. 관리자 재계산도 동일(총=n(n+1)/2)
- **쿠폰**: 바코드 삭제됨(매장 인증 코드 입력만)
- **연출**: 응모완료(샤로록 접수) 3초 → 시크릿 굿즈 팝업(최초 1회, 카운트다운 삭제). 스탬프 도장=새 파란 샤도장(stamp_sha2.svg, 76px). 키캡 130px. 히어로=hero2.svg(날짜 수정본)
- 메인 환영문 첫 줄: "샤로수길에서만 즐길 수 있는 특별한 빙고투어가 진행됩니다." / 전체 word-break:keep-all
...
- `POST /api/state/get` {phone} — 진행상황 조회 (state.v 타임스탬프 비교로 최신분만 클라이언트 반영; 로드 시+visibilitychange 시 sync)
...
- 관리자(헤더 `x-admin-key` 필요):
  ...
  - `POST /api/admin/user/state` {phone, state} — 참여자 빙고판 강제 수정(admin_edit 이벤트 기록). 관리자 UI에서 v=Date.now()로 저장해 참여자 기기가 자동 수용
...
참여자 행 "빙고판" 버튼: 3×3 그리드에서 칸 토글(채움/지움) 후 저장 → 참여자 기기 자동 반영 (이용기록에 '관리자 수정' 기록)
관리자 저장 시 완성 줄 **전체 재계산**: tickets = min(완성 줄 수, 8). 채우면 응모권 자동 추가, **지워서 줄이 깨지면 응모권도 회수**. 증감분은 bingo_line/ticket 이벤트로 기록(음수 = 회수). 참여자 테이블·CSV의 스탬프/줄/응모권 수치는 state 기준(실시간)
