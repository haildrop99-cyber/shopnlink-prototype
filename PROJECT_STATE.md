...
- 마지막 티켓 응모 후: 3→2→1 카운트다운("행운이 찾아가는 중…") → **시크릿 경품 도착(🎁 CLICK)** → "축하합니다! 완성한 빙고 N줄이 실물 키캡으로 전환되었습니다! (주 1회 일괄 발송)"
- 3×3 전체 완성: "빙고 완성!! 샤로수길 전문가 인정" 도장 연출 → 티켓 플로우
- 효과: 클리커 사운드(WebAudio 880Hz triangle), 이모지 컨페티

### 진행상황 저장 (localStorage)
- `snl_user` = {name, phone, address} / `snl_state` = {mode, board, stamped, doneLines, tickets}
- 재접속: `restoreSession()` — 유저 있으면 폼 프리필, 상태 있으면 **바로 빙고판 복원**, 없으면 course로
